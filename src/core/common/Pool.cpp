...
Pool::Pool(positive maxVerticesCount, positive maxFacesCount, positive maxLightsCount)
    : vertexPool(new real[maxVerticesCount * VEC4_SCALARS_COUNT]),
      ...
      mappingPool(new real[maxVerticesCount * VEC2_SCALARS_COUNT]), currentVerticesCount(0),
      facePool(new positive[maxFacesCount * 4]), currentFacesCount(0),
      lightPool(new Light*[maxLightsCount]), currentLightsCount(0)
{
	// ctor
}
