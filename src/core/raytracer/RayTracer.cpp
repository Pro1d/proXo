...
RayTracer::RayTracer(Buffer* imageBuffer, Scene* scene)
    : scene(scene), pool(NULL), tree(NULL), imageBuffer(imageBuffer), multithread(THREADS_COUNT),
...
#include <cstdio>
positive RayTracer::getColor(vec3 orig, vec3 dir, real currentRefractiveIndex, real maxIntensity,
    positive* lastFace, vec3 colorOut, real* depthOut, TreeStack& stack)
{
	...
	if(intersect.intersectionSide != 0) {
		...
		real point[VEC4_SCALARS_COUNT] = { Ka * pool->vertexPool[Ia * VEC4_SCALARS_COUNT + 0]
		...
		/// Get material
		Material mat; // ambient=occlusion ambient, diffuse=light, specular/shininess=light_reflect,
		              // reflect=%, refractiveIndex=, aepthAbsortion=exp(dist*.)

		real color[VEC3_SCALARS_COUNT] = { Ka * Ma[MAT_POOL_INDEX_RED] + Kb * Mb[MAT_POOL_INDEX_RED]
			    + Kc * Mc[MAT_POOL_INDEX_RED],
		...
		mat.ambient = Ka * Ma[MAT_POOL_INDEX_AMBIENT] + Kb * Mb[MAT_POOL_INDEX_AMBIENT]
		    + Kc * Mc[MAT_POOL_INDEX_AMBIENT];
		mat.diffuse = Ka * Ma[MAT_POOL_INDEX_DIFFUSE] + Kb * Mb[MAT_POOL_INDEX_DIFFUSE]
		    + Kc * Mc[MAT_POOL_INDEX_DIFFUSE];
		mat.specular = Ka * Ma[MAT_POOL_INDEX_SPECULAR] + Kb * Mb[MAT_POOL_INDEX_SPECULAR]
		    + Kc * Mc[MAT_POOL_INDEX_SPECULAR];
		mat.shininess = Ka * Ma[MAT_POOL_INDEX_SHININESS] + Kb * Mb[MAT_POOL_INDEX_SHININESS]
		    + Kc * Mc[MAT_POOL_INDEX_SHININESS];
		mat.emissive = Ka * Ma[MAT_POOL_INDEX_EMISSIVE] + Kb * Mb[MAT_POOL_INDEX_EMISSIVE]
		    + Kc * Mc[MAT_POOL_INDEX_EMISSIVE];
		mat.reflect = Ka * Ma[MAT_POOL_INDEX_REFLECT] + Kb * Mb[MAT_POOL_INDEX_REFLECT]
		    + Kc * Mc[MAT_POOL_INDEX_REFLECT];
		mat.refractiveIndex = Ka * Ma[MAT_POOL_INDEX_REFRACTIVE]
		    + Kb * Mb[MAT_POOL_INDEX_REFRACTIVE] + Kc * Mc[MAT_POOL_INDEX_REFRACTIVE];
		mat.depthAbsorbtion = Ka * Ma[MAT_POOL_INDEX_ABSORPTION]
		    + Kb * Mb[MAT_POOL_INDEX_ABSORPTION] + Kc * Mc[MAT_POOL_INDEX_ABSORPTION];
		/// Normal
		real normal[VEC4_SCALARS_COUNT] = { Ka * pool->normalPool[Ia * VEC4_SCALARS_COUNT + 0]
		...
		// lights/shadow // depthAbsorption == +inf && reflect < 1 ; specular=reflect
		...
		if(mat.depthAbsorbtion > 1 - EPSILON)
			for(positive i = 0; i < pool->currentLightsCount; i++) {
				...
				if(pool->lightPool[i]->castShadow) {
					...
					intersectTree(point, lightDir, tree, stack, pool->vertexPool, intersect.face,
					    intersectLight); // TODO find one face (no ordering needed)
					shadow = intersectLight.intersectionSide != 0 && intersectLight.depth < dist;
				}
				if(!shadow) {
					// lighting!
					pool->lightPool[i]->lighting(color, normal, point, mat.ambient, mat.diffuse,
					    mat.specular, mat.shininess, colorOut);
		...
		if(mat.depthAbsorbtion < (1 - EPSILON)) {
			refractRatio =
			    refractRay(dir, normal, currentRefractiveIndex, mat.refractiveIndex, refractDir);
		...
		if(reflectIntensity > EPSILON) {
			...
			real reflectDepth;
			getColor(point, reflectDir, currentRefractiveIndex, reflectIntensity * maxIntensity,
			    intersect.face, reflectColor, &reflectDepth, stack);
