...
void DepthOfField::updatePattern(positive firstAngleStepCount, positive radiusStepCount)
{
	origins_.resize(firstAngleStepCount * radiusStepCount*(radiusStepCount+1)/2);
	positive i = 0;
	for(positive r = 1; r <= radiusStepCount; r++) {
		...
		for(positive a = 0; a < angleStepCount; a++) {
			...
			origins_[i].second = sin(a * 2*M_PI / angleStepCount) * radius;
			i++;
		}
	}
}
...
void DepthOfField::getDirection(vec3 mainDir, OriginsIterator oi, vec4 outDir, vec4 outOrig)
{
	outOrig[0] = oi->first;
	outOrig[1] = oi->second;
	outOrig[2] = 0;
	...
	real focusPoint[VEC4_SCALARS_COUNT] = {
		mainDir[0] * focusDistance_,
		mainDir[1] * focusDistance_,
		mainDir[2] * focusDistance_,
		1
	};
