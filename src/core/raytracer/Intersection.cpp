...
void pushSubTree(vec3 orig, vec3 dir, KDTree * tree, TreeStack & stack) {
    real distFirst = intersectAxialParallelepiped(orig, dir, tree->firstSubTree->bounds+BOUND_X_MIN, tree->firstSubTree->bounds+BOUND_X_MAX);
    real distSecond = intersectAxialParallelepiped(orig, dir, tree->secondSubTree->bounds+BOUND_X_MIN, tree->secondSubTree->bounds+BOUND_X_MAX);
...
void intersectTree(vec3 orig, vec3 dir, KDTree * root, TreeStack & stack, vec4 vertices, positive * faceToIgnore, IntersectionData & out) {
    KDTree ** empty = stack.treeStackTop;
    out.intersectionSide = 0;
    out.depth = std::numeric_limits<real>::infinity();

    real distRoot = intersectAxialParallelepiped(orig, dir, root->bounds, root->bounds+BOUND_X_MAX);
    if(distRoot < 0)
        return;
    ...
    while(stack.treeStackTop != empty) {
        ...
        if(depth < out.depth) {
            if(node->isLeaf) {
                intersectSetOfTriangles(orig, dir, node->faces, node->facesCount, vertices, faceToIgnore, out);
            }
            else {
