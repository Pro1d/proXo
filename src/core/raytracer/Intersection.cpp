...
integer intersectTriangle(
    vec3 orig, vec3 dir, vec3 vert0, vec3 vert1, vec3 vert2, real* t, real* u, real* v)
{
...
void pushSubTree(vec3 orig, vec3 dir, KDTree* tree, TreeStack& stack)
{
	real distFirst = intersectAxialParallelepiped(orig, dir,
	    tree->firstSubTree->bounds + BOUND_X_MIN, tree->firstSubTree->bounds + BOUND_X_MAX);
	real distSecond = intersectAxialParallelepiped(orig, dir,
	    tree->secondSubTree->bounds + BOUND_X_MIN, tree->secondSubTree->bounds + BOUND_X_MAX);
	...
	if(tree->middleSubTree != NULL) {
		real distMiddle = intersectAxialParallelepiped(orig, dir,
		    tree->middleSubTree->bounds + BOUND_X_MIN, tree->middleSubTree->bounds + BOUND_X_MAX);
...
void intersectSetOfTriangles(vec3 orig, vec3 dir, positive* faces, positive facesCount,
    vec4 vertices, positive* faceToIgnore, IntersectionData& out)
{
	for(positive i = 0; i < facesCount; i++) {
		...
		integer intersect = intersectTriangle(orig, dir, v1, v2, v3, &t, &u, &v);
		if(intersect != 0 && 0 < t && t < out.depth && faces + i * 4 != faceToIgnore) {
...
void intersectTree(vec3 orig, vec3 dir, KDTree* root, TreeStack& stack, vec4 vertices,
    positive* faceToIgnore, IntersectionData& out)
{
	...
	real distRoot =
	    intersectAxialParallelepiped(orig, dir, root->bounds, root->bounds + BOUND_X_MAX);
	if(distRoot < 0)
		return;
	...
	while(stack.treeStackTop != empty) {
		...
		/// Testi if this node can contain faces closer than the actual nearest face.
		if(depth < out.depth) {
			if(node->isLeaf) {
				intersectSetOfTriangles(
				    orig, dir, node->faces, node->facesCount, vertices, faceToIgnore, out);
			}
			else {
