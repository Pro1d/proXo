...
void bufferToBitmap24bppOpt(Buffer & buffer, SDL_Surface * bmp, positive sampleSize) {
    ...
    const positive eW = (std::min(buffer.width >> sampleSizeExp, (positive) bmp->w) << sampleSizeExp) * VEC4_SCALARS_COUNT;
    const positive eH = (std::min(buffer.height >> sampleSizeExp, (positive) bmp->h) << sampleSizeExp);
	...
	for(positive ey = 0, eyEnd = sampleSize; ey < eH; eyEnd += sampleSize, pixelLine += bmp->pitch) {
		memset(line, 0, bW);
		
		for(; ey < eyEnd; ey++, dataLine += bufferPitch) {
			const vec4 dataEnd = dataLine + eW;
			for(vec4 data = dataLine, dataSEnd = data + sS, it = line; data < dataEnd; it += VEC4_SCALARS_COUNT, dataSEnd += sS) {
				while(data < dataSEnd) {
					it[0] += data[0];
					it[1] += data[1];
					it[2] += data[2];
		...
		for(vec4 it = line; it < endLine; it += VEC4_SCALARS_COUNT) {
            if(SDL_BYTEORDER == SDL_BIG_ENDIAN) {
                pixel[0] = it[0] <= 0 ? 0 : std::min((positive) (it[0] * 255) >> samplePixelsCountExp, (positive) 255);
                pixel[1] = it[1] <= 0 ? 0 : std::min((positive) (it[1] * 255) >> samplePixelsCountExp, (positive) 255);
                pixel[2] = it[2] <= 0 ? 0 : std::min((positive) (it[2] * 255) >> samplePixelsCountExp, (positive) 255);
            } else {
                pixel[0] = it[0] <= 0 ? 0 : std::min((positive) (it[0] * 255) >> samplePixelsCountExp, (positive) 255);
                pixel[1] = it[1] <= 0 ? 0 : std::min((positive) (it[1] * 255) >> samplePixelsCountExp, (positive) 255);
                pixel[2] = it[2] <= 0 ? 0 : std::min((positive) (it[2] * 255) >> samplePixelsCountExp, (positive) 255);
...
void BufferToBitmap::bufferToBitmap24bppThread(void * data, positive threadId, positive threadsCount)
{
    ...
    const positive bufferWidth = std::min(buffer.width >> sampleSize_log2, (positive) bitmap->w) << sampleSize_log2;
    ...
    for(positive sy = yStart; sy < yEnd; sy += sampleSize)
	{
        ...
        for(positive x = 0; x < bufferWidth; x += sampleSize)
        {
			...
			vec4 bufferDataLineEnd = bufferDataLine + (sampleSize * buffer.width * VEC4_SCALARS_COUNT);
            for(; bufferDataLine < bufferDataLineEnd; bufferDataLine += buffer.width*VEC4_SCALARS_COUNT)
			{
            ...
            Uint8 r = color[0] <= 0 ? 0 : std::min((positive) (color[0] * 255) >> samplePixelsCount_log2, (positive) 255);
            Uint8 g = color[1] <= 0 ? 0 : std::min((positive) (color[1] * 255) >> samplePixelsCount_log2, (positive) 255);
            Uint8 b = color[2] <= 0 ? 0 : std::min((positive) (color[2] * 255) >> samplePixelsCount_log2, (positive) 255);
...
BufferToBitmap::BufferToBitmap(Buffer & buffer, SDL_Surface * bmp, positive sampleSize) : multithread(THREADS_COUNT), buffer(&buffer), bitmap(bmp), sampleSize(sampleSize)
{
